...
gulp.task("sass-compile", function() {
  return gulp
    ...
    .pipe(
      sass({
        includePaths: [paths.app + "styles/extras/", themes.current.path]
      })
    )
    .pipe(gulp.dest(paths.tmp));
});
...
gulp.task("emoji", function(cb) {
  ...
  emojis.forEach(function(emoji) {
    Jimp.read(emojiPath + "img-twitter-64/" + emoji.image, function(
      err,
      lenna
    ) {
      if (err) throw err;
...
gulp.task(
  "app-deploy",
  ["coffee", "conf", "locales", "moment-locales", "app-loader"],
  function() {
    return gulp
...
gulp.task("copy-prism", ["prism-languages"], function() {
  ...
  return gulp
    .src(prismLanguages.concat(__dirname + "/prism-languages.json"))
...
gulp.task("express", function() {
  ...
  app.use(
    "/" + version + "/prism",
    express.static(__dirname + "/dist/" + version + "/prism")
  );
